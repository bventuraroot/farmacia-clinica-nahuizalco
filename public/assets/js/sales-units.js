...
$(document).ready(function() {
    ...
    function initializeSalesUnits() {
        ...
        // Evento para búsqueda por nombre - ELIMINADO (duplicado con sales-dynamic.js)
        // La funcionalidad se maneja desde sales-dynamic.js
        ...
        $(document).on('change', '#unit-select', function() {
            ...
            setTimeout(function() {
                // Recalcular validaciones cuando cambie la unidad usando los datos del producto actual
                if (currentProductData && currentProductData.product && currentProductData.stock) {
                    const product = currentProductData.product;
                    const stock = currentProductData.stock;
                    const quantity = parseFloat($('#cantidad').val()) || 0;
                    const availableStock = parseFloat(stock.base_quantity) || 0;
                    const saleType = product.sale_type || 'unit';


                    let remainingAfterSale = availableStock;
                    let remainingInMeasure = 0;

                    if (saleType === 'weight') {
                        const weightPerUnit = parseFloat(product.weight_per_unit) || 100;
                        ...
                        if (unitCode === '36') { // Código de libra
                            ...
                            remainingInMeasure = remainingAfterSale * weightPerUnit;
                        } else {
                            remainingAfterSale = availableStock - quantity;
                            remainingInMeasure = remainingAfterSale * weightPerUnit;
                        }

                        // $('#catalog-validation-after').text(Math.max(0, remainingAfterSale).toFixed(4) + ' sacos'); // COMENTADO para evitar conflictos
                        // $('#catalog-validation-lbs').text(Math.max(0, remainingInMeasure).toFixed(4) + ' libras'); // COMENTADO para evitar conflictos

                    } else if (saleType === 'volume') {
                        const volumePerUnit = parseFloat(product.volume_per_unit) || 25;

                        if (unitCode === '23') { // Código de litro
                            const quantityInContainers = quantity / volumePerUnit;
                            remainingAfterSale = availableStock - quantityInContainers;
                            remainingInMeasure = remainingAfterSale * volumePerUnit;
                        } else {
                            remainingAfterSale = availableStock - quantity;
                            remainingInMeasure = remainingAfterSale * volumePerUnit;
                        }

                        // $('#catalog-validation-after').text(Math.max(0, remainingAfterSale).toFixed(4) + ' contenedores'); // COMENTADO para evitar conflictos
                        // $('#catalog-validation-lbs').text(Math.max(0, remainingInMeasure).toFixed(4) + ' litros'); // COMENTADO para evitar conflictos

                    } else {
                        remainingAfterSale = availableStock - quantity;
                        remainingInMeasure = remainingAfterSale;

                        // $('#catalog-validation-after').text(Math.max(0, remainingAfterSale).toFixed(4) + ' unidades'); // COMENTADO para evitar conflictos
                        // $('#catalog-validation-lbs').text(Math.max(0, remainingInMeasure).toFixed(4) + ' unidades'); // COMENTADO para evitar conflictos
                    }
        ...
        $(document).on('change keyup', '#cantidad, #precio', function() {
            // updatePanelWithCurrentData(); // Función obsoleta
        ...
        $(document).on('change keyup', '#cantidad', function() {
            ...
            $('#catalog-validation-sell').text(quantity.toFixed(2));
                        ...
                        // Forzar actualización inmediata de todas las validaciones
            updateValidationsForCurrentUnit(quantity);

            // También actualizar el campo "A vender" directamente
            $('#catalog-validation-sell').text(quantity.toFixed(2));

            // Recalcular stock después de la venta usando los datos del producto actual
            if (currentProductData && currentProductData.product && currentProductData.stock) {
                const product = currentProductData.product;
                const stock = currentProductData.stock;
                const availableStock = parseFloat(stock.base_quantity) || 0;
                ...
                if (saleType === 'weight') {
                    ...
                    $('#catalog-validation-after').text(Math.max(0, remainingAfterSale).toFixed(4) + ' sacos');
                    $('#catalog-validation-lbs').text(Math.max(0, remainingInMeasure).toFixed(4) + ' libras');
                    ...
                    $('#catalog-validation-after').text(Math.max(0, remainingAfterSale).toFixed(4) + ' contenedores');
                    $('#catalog-validation-lbs').text(Math.max(0, remainingInMeasure).toFixed(4) + ' litros');

                } else {
                    remainingAfterSale = availableStock - quantity;
                    remainingInMeasure = remainingAfterSale;

                    $('#catalog-validation-after').text(Math.max(0, remainingAfterSale).toFixed(4) + ' unidades');
                    $('#catalog-validation-lbs').text(Math.max(0, remainingInMeasure).toFixed(4) + ' unidades');
                }
        ...
        // Función para actualizar validaciones según la unidad actual
        function updateValidationsForCurrentUnit(quantity) {
            if (!currentProductData || !currentProductData.product || !currentProductData.stock) {
                return;
            }
    ...
    function loadUnitsDropdown(units) {
        ...
        if (units && units.length > 0) {
            ...
            const allowedUnits = units.filter(function(unit) {
                const isAllowed = allowedUnitCodes.includes(unit.unit_code);
                return isAllowed;
            });
    ...
    function updateConversionDisplay(data) {
        ...
        const weightPerUnit = parseFloat(data.calculations?.weight_per_unit) || 0;
        const pricePerSack = parseFloat(data.calculations?.price_per_sack) || 0;
        const pricePerPound = parseFloat(data.calculations?.price_per_pound) || 0;
        const pricePerLiter = parseFloat(data.calculations?.price_per_liter) || 0;
        const volumePerUnit = parseFloat(data.calculations?.volume_per_unit) || 0;

        // Determinar el tipo de producto para mostrar la información correcta
        ...
        if (measureType === 'volume') {
            conversionDetails = `
                <div class="conversion-details">
                    <div class="conversion-row">
                        <div class="conversion-label">Precio del contenedor:</div>
                        <div class="conversion-value">$${pricePerSack.toFixed(2)}</div>
                    </div>
                    <div class="conversion-row">
                        <div class="conversion-label">Litros por contenedor:</div>
                        <div class="conversion-value">${volumePerUnit.toFixed(4)} litros</div>
                    </div>
                    <div class="conversion-row">
                        <div class="conversion-label">Precio por litro:</div>
                        <div class="conversion-value">$${pricePerLiter.toFixed(2)}</div>
                    </div>
                    <div class="conversion-row">
            ...
            conversionDetails = `
                <div class="conversion-details">
                    <div class="conversion-row">
                        <div class="conversion-label">Precio por unidad:</div>
                        <div class="conversion-value">$${pricePerSack.toFixed(2)}</div>
                    </div>
                    <div class="conversion-row">
                        <div class="conversion-label">Subtotal:</div>
...
// window.updatePanelWithCurrentData = updatePanelWithCurrentData; // Función obsoleta
...
function updateFixedCatalogCards(productData) {
    ...
    if (productData && productData.product) {
        ...
        // Convertir todos los valores a números para evitar errores
        const price = parseFloat(product.price) || 0;
        const weightPerUnit = parseFloat(product.weight_per_unit) || 0;
        const volumePerUnit = parseFloat(product.volume_per_unit) || 0;
        const baseQuantity = parseFloat(stock?.base_quantity) || 0;
        const baseUnit = stock?.base_unit || 'unidades';

                // Intentar obtener sale_type del producto, si no está disponible, usar measure_type del stock
        let saleType = product.sale_type;
        if (!saleType && stock && stock.measure_type) {
            saleType = stock.measure_type;
        }
        ...
        // IMPORTANTE: Siempre usar el precio base del producto, no el precio de la unidad seleccionada
        const baseProductPrice = parseFloat(product.original_price || product.base_price || product.price) || price;
        $('#catalog-price-sack').text('$' + baseProductPrice.toFixed(2));

        // Actualizar etiquetas y valores según el tipo de producto
        if (saleType === 'weight') {
            $('#catalog-weight-total').text(weightPerUnit.toFixed(4) + ' libras');
            $('#catalog-price-pound').text('$' + (baseProductPrice / (weightPerUnit || 1)).toFixed(2));
            ...
            $('#catalog-price-pound').text('$' + (baseProductPrice / (volumePerUnit || 1)).toFixed(2));
            ...
            $('#catalog-price-pound').text('$' + price.toFixed(2));
        ...
        $('#catalog-subtotal').text('$' + baseProductPrice.toFixed(2));

        // Tarjeta de Stock Disponible
        $('#catalog-stock-available').text(baseQuantity.toFixed(4) + ' ' + baseUnit);

        // Actualizar stock según el tipo de producto
        if (saleType === 'weight') {
            $('#catalog-stock-lbs').text((baseQuantity * weightPerUnit).toFixed(4) + ' libras');
            $('#catalog-stock-lbs-label').text('Total en Libras');
        } else if (saleType === 'volume') {
            $('#catalog-stock-lbs').text((baseQuantity * volumePerUnit).toFixed(4) + ' litros');
            $('#catalog-stock-lbs-label').text('Total en Litros');
        } else {
            $('#catalog-stock-lbs').text(baseQuantity.toFixed(4) + ' unidades');
            $('#catalog-stock-lbs-label').text('Total en Unidades');
        }

        let stockStatusFixed = baseQuantity > 0 ?
            '<span class="badge bg-success">Disponible</span>' :
            '<span class="badge bg-danger">Sin stock</span>';
        $('#catalog-stock-status').html(stockStatusFixed);

        // Tarjeta de Validaciones
        $('#catalog-validation-current').text(baseQuantity.toFixed(4) + ' ' + baseUnit);
        $('#catalog-validation-sell').text('0');
        $('#catalog-validation-after').text(baseQuantity.toFixed(4) + ' ' + baseUnit);

        // Actualizar validaciones según el tipo de producto
        if (saleType === 'weight') {
            $('#catalog-validation-lbs').text((baseQuantity * weightPerUnit).toFixed(4) + ' libras');
            $('#catalog-validation-lbs-label').text('Stock en Libras');
        } else if (saleType === 'volume') {
            $('#catalog-validation-lbs').text((baseQuantity * volumePerUnit).toFixed(4) + ' litros');
            $('#catalog-validation-lbs-label').text('Stock en Litros');
        } else {
            $('#catalog-validation-lbs').text(baseQuantity.toFixed(4) + ' unidades');
            $('#catalog-validation-lbs-label').text('Stock en Unidades');
        }
...
function updateFixedCatalogCardsFromConversion(data) {
    ...
    if (data) {
        ...
        const pricePerPound = parseFloat(data.calculations?.price_per_pound) || 0;
        const pricePerLiter = parseFloat(data.calculations?.price_per_liter) || 0;
        const basePrice = parseFloat(data.calculations?.base_price) || 0;
        const subtotal = parseFloat(data.subtotal) || 0;
        const availableQuantity = parseFloat(data.stock_info?.available_quantity) || 0;
        const baseUnit = data.stock_info?.base_unit || 'unidades';
        const totalInLbs = parseFloat(data.stock_info?.total_in_lbs) || 0;
        const totalInLiters = parseFloat(data.stock_info?.total_in_liters) || 0;
        const measureType = data.stock_info?.measure_type || 'unit';
        ...
        if (measureType === 'weight') {
            ...
            $('#catalog-weight-total').text('0.0000 unidades');
            $('#catalog-price-pound').text('$' + basePrice.toFixed(2));
        ...
        $('#catalog-subtotal').text('$' + subtotal.toFixed(2));

        // Tarjeta de Stock Disponible
        $('#catalog-stock-available').text(availableQuantity.toFixed(4) + ' ' + baseUnit);

        // Actualizar stock según el tipo de producto
        if (measureType === 'weight') {
            $('#catalog-stock-lbs').text(totalInLbs.toFixed(4) + ' libras');
            $('#catalog-stock-lbs-label').text('Total en Libras');
        } else if (measureType === 'volume') {
            $('#catalog-stock-lbs').text(totalInLiters.toFixed(4) + ' litros');
            $('#catalog-stock-lbs-label').text('Total en Litros');
        } else {
            $('#catalog-stock-lbs').text(availableQuantity.toFixed(4) + ' unidades');
            $('#catalog-stock-lbs-label').text('Total en Unidades');
        }
        ...
        let remainingAfterSale = availableQuantity;
        let remainingInMeasure = 0;
        let isAvailable = true;
        ...
        if (measureType === 'weight') {
            if (unitCode === '36') { // Código de libra
                quantityInBaseUnits = quantity / weightPerUnit;
                remainingAfterSale = availableQuantity - quantityInBaseUnits;
                remainingInMeasure = remainingAfterSale * weightPerUnit;
            } else {
                remainingAfterSale = availableQuantity - quantity;
                remainingInMeasure = remainingAfterSale * weightPerUnit;
            }
        } else if (measureType === 'volume') {
            if (unitCode === '99') { // Código de litro
                quantityInBaseUnits = quantity / volumePerUnit;
                remainingAfterSale = availableQuantity - quantityInBaseUnits;
                remainingInMeasure = remainingAfterSale * volumePerUnit;
            } else {
                remainingAfterSale = availableQuantity - quantity;
        ...
        const displayRemainingInMeasure = Math.max(0, remainingInMeasure);

        $('#catalog-validation-current').text(availableQuantity.toFixed(4) + ' ' + baseUnit);
        $('#catalog-validation-sell').text(quantity.toFixed(2));
        $('#catalog-validation-after').text(displayRemainingAfterSale.toFixed(4) + ' ' + baseUnit);

        // Actualizar validaciones según el tipo de producto
        if (measureType === 'weight') {
            $('#catalog-validation-lbs').text(displayRemainingInMeasure.toFixed(4) + ' libras');
            $('#catalog-validation-lbs-label').text('Stock en Libras');
        } else if (measureType === 'volume') {
            $('#catalog-validation-lbs').text(displayRemainingInMeasure.toFixed(4) + ' litros');
            $('#catalog-validation-lbs-label').text('Stock en Litros');
        } else {
            $('#catalog-validation-lbs').text(displayRemainingInMeasure.toFixed(4) + ' unidades');
            $('#catalog-validation-lbs-label').text('Stock en Unidades');
        }
...
// Hacer funciones disponibles globalmente - COMENTADO TEMPORALMENTE
// window.loadProductUnitsInternal = loadProductUnitsInternal;
// window.loadUnitsDropdown = loadUnitsDropdown;

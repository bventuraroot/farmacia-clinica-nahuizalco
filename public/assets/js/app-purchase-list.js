...
function addProductRowWithData(product) {
    ...
    // Cargar unidades disponibles para este producto
    loadProductUnits(productRowIndex, product.id);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.data.units) {
                ...
                const unitSelect = $(`#productRow_${rowIndex} .unit-select`);

                // Limpiar opciones existentes
                unitSelect.empty().append('<option value="">Seleccionar unidad...</option>');

                // Agregar unidades disponibles
                units.forEach(unit => {
                    const unitName = getUnitDisplayName(unit.unit_code, unit.unit_name);
                    unitSelect.append(`<option value="${unit.unit_code}" data-id="${unit.unit_id}" data-factor="${unit.conversion_factor}">${unitName}</option>`);
                });

...
function updateAllInformationCards(productData, unitCode) {
    ...
    if (measureType === 'weight') {
        ...
        // Producto por unidad
        $('#catalog-stock-lbs').text('0.0000 unidades');
    ...
    // Calcular cantidad a vender y stock después
    const cantidadAVender = parseFloat($('#cantidad').val()) || 0;
    const stockDespues = Math.max(0, stockQuantity - cantidadAVender);

    $('#catalog-validation-sell').text(cantidadAVender.toFixed(2));
    $('#catalog-validation-after').text(stockDespues.toFixed(4) + ' ' + stockUnit);
...
function calculateProductConversionsAndStock(productId, unitCode, quantity) {
    ...
    if (productData.sale_type === 'volume' && unitCode === '23') {
        ...
        // Otras unidades o productos por unidad
                stockDespues = Math.max(0, stockData.base_quantity - quantity);
        stockEnMedida = stockDespues;
    }

    // 3. ACTUALIZAR TARJETA DE VALIDACIONES
    $('#catalog-validation-sell').text(quantity.toFixed(2));
    $('#catalog-validation-after').text(stockDespues.toFixed(4) + ' ' + $('#unit-select option:selected').text());
        ...
        success: function(response) {
            if (response.success && response.products && response.products.length > 0) {
                ...
                response.products.forEach(function(product, index) {
                    ...
                    // Construir descripción completa: nombre + marca (como en agregarp)
                    const descriptionbyproduct = product.product_name + " " + (product.marca_name || product.marca || '');
...
function initializeSalesDynamic() {
    ...
    $('#cantidad').on('change', function() {
        ...
        if (productId && productId !== '') {
            // Actualizar los cuadros de información con la nueva cantidad
            const currentProductData = {
                ...
                stock: {
                    base_quantity: parseFloat($('#stock_quantity').val()) || 0,
                    unit_name: $('#unit-select option:selected').text() || 'unidades',
            ...
            // Actualizar los cuadros con la nueva cantidad
            updateAllInformationCards(currentProductData, $('#unit-select').val());
    ...
    $('#unit-select').on('change', function() {
        ...
        if (productId && productId !== '' && unitCode && unitCode !== '') {
            ...
            const currentProductData = {
                ...
                stock: {
                    base_quantity: parseFloat($('#stock_quantity').val()) || 0,
                    unit_name: $('#unit-select option:selected').text(),
                    total_in_lbs: parseFloat($('#total_in_lbs').val()) || 0,
                    total_in_liters: parseFloat($('#total_in_liters').val()) || 0,
                    total_in_ml: parseFloat($('#total_in_ml').val()) || 0,
                    measure_type: $('#measure_type').val() || 'unit'
...
function setupEventListeners() {
    // Evento para búsqueda por código - Solo al presionar Enter
    ...
    // Evento para limpiar cuando el campo se vacíe
...
function searchproductcode(codeproduct) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Cargar información de stock del producto (igual que en searchproduct)
            if (response && response.length > 0) {
                loadProductStock(response[0].id);
            }
            ...
            if (response && response.length > 0) {
                ...
                if (typeof loadProductUnitsInternal === 'function') {
                    loadProductUnitsInternal(response[0].id);
                } else {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.data && response.data.units) {
                ...
                if (response.data.product && response.data.product.sale_type) {

...
class DteController extends Controller
{
    ...
    public function contingencias(Request $request)
    {
        ...
        $contingencias = $query->orderBy('created_at', 'desc')->paginate(15);
    ...
    public function crearContingencia(Request $request)
    {
        try {
            ...
            $contingencia->empresa_id = $request->company;
            $contingencia->idEmpresa = $request->company; // Para compatibilidad legacy
            ...
            $contingencia->selloRecibido = null; // Inicialmente vacío para permitir autorización
            ...
            Log::error('Error al crear contingencia: ' . $e->getMessage());
    ...
    public function autorizarContingenciaGet($empresaId, $contingenciaId)
    {
        ...
        try {
            ...
            LEFT JOIN companies b ON a.empresa_id = b.id OR a.idEmpresa = b.id
            INNER JOIN phones c ON b.phone_id = c.id
            WHERE (a.empresa_id = $id_empresa OR a.idEmpresa = $id_empresa) AND a.id = $contingenciaId";
            ...
            Log::error('Error al autorizar contingencia: ' . $e->getMessage());
            $tipo_resultado = "danger";
            $mensaje_resultado = 'Error al autorizar contingencia: ' . $e->getMessage();
        }
    ...
    public function mostrarErroresContingencia($contingenciaId)
    {
        try {
            ...
            // Errores de ventas relacionadas
            $ventasConError = Sale::where('id_contingencia', $contingenciaId)
                ->whereNotNull('error_message')
                ->get();

            foreach ($ventasConError as $venta) {
                $errores->push([
                    'tipo' => 'Venta',
                    'venta_id' => $venta->id,
                    'mensaje' => $venta->error_message,
                    'fecha' => $venta->updated_at,
                    'severidad' => 'error'
                ]);
            }

            // Errores de DTE si existen
            $dtesConError = Dte::where('contingencia_id', $contingenciaId)
                ->whereNotNull('error_message')
                ->get();
            ...
            foreach ($dtesConError as $dte) {
                $errores->push([
                    'tipo' => 'DTE',
                    'dte_id' => $dte->id,
                    'mensaje' => $dte->error_message,
                    'fecha' => $dte->updated_at,
                    'severidad' => 'error'
                ]);
            }
            ...
            Log::error('Error al obtener errores de contingencia: ' . $e->getMessage());

            if (request()->ajax() || request()->wantsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Error al obtener errores: ' . $e->getMessage()
                ], 500);
            }

...
@section('page-script')
    <script src="{{ asset('assets/js/app-contingencia-list.js') }}"></script>
    <script src="{{ asset('assets/js/forms-contingencia.js') }}"></script>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-header border-bottom">
            <h5 class="mb-3 card-title">Contingencias</h5>
        ...
        <div class="card-datatable table-responsive">
            <table class="table datatables-provider border-top">
                <thead>
                    <tr>
                        <th>Ver</th>
                        <th width="15%">Fecha Inicio - Fecha Final</th>
                ...
                <tbody>
                    @isset($contingencias)
                        @forelse($contingencias as $d)
                            <tr  @if (isset($d->desTransaccion) && $d->desTransaccion == "Invalidacion") style="color:red;" @endif>
                                <td></td>
                                <td>{{ $d->fInicio ?? 'N/A' }} - {{ $d->fFin ?? 'N/A' }}</td>
                                <td>Codigo Generacion: {{ $d->codigoGeneracion ?? 'N/A' }}<br>
                                    Sello: {{ $d->selloRecibido ?? 'N/A' }}<br>
                                    Fecha Recibido: {{ $d->fhRecibido ?? 'N/A' }}
                                </td>
                                <td>{{ $d->estado ?? 'N/A' }}</td>
                                <td>{{ $d->descripcionMsg ?? 'N/A' }}</td>
                                <td>{!! $d->observacionesMsg ?? 'N/A' !!}</td>
                                <td><div class="d-flex align-items-center">
                                    @if (is_null($d->selloRecibido ?? null))
                                    <a href="{{route('factmh.autoriza_contingencia',['empresa'=> $d->idEmpresa ?? 0, 'id' => $d->id ?? 0 ])}}" style="margin-right: 10%" alt="Autorizar"><i class="fas fa-paper-plane" alt="Autorizar"></i></a>
                                    @endif
                                    @if (($d->codEstado ?? '') == '02')
                                    <a href="{{route('factmh.procesa_contingencia', $d->id ?? 0)}}"><i class="fas fa-file-invoice" alt="Procesar"></i></a>
                                    @endif
                                    <a href="{{route('factmh.muestra_lote', $d->id ?? 0)}}"><i class="fas fa-eye" alt="Mostrar Lote"></i></a>
                                    </div>
                                </div></td>
                            </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center">
                                        <div class="alert alert-info">
                                            ...
                                            No hay contingencias registradas
                                        </div>
                                    </td>
                                </tr>
                            @endforelse

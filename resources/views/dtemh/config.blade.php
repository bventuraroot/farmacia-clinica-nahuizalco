...
$(document).ready(function() {
    ...
    window.editconfig = function(id) {
        $.ajax({
            url: '{{ route("config.get", ":id") }}'.replace(':id', btoa(id)),
            type: 'GET',
            success: function(response) {
                if (response && response.length > 0) {
                    ...
                    $('#idedit').val(config.id);
                    $('#versionedit').val(config.version);
                    $('#ambienteedit').val(config.ambiente).trigger('change');
                    $('#typemodeledit').val(config.typeModel);
                    $('#typetransmissionedit').val(config.typeTransmission);
                    $('#typecontingenciaedit').val(config.typeContingencia);
                    $('#versionjsonedit').val(config.versionJson).trigger('change');
                    $('#passprivatekeyedit').val(config.passPrivateKey);
                    $('#passpublickeyedit').val(config.passkeyPublic);
                    $('#passmhedit').val(config.passMH);
                    $('#dte_emission_notes_edit').val(config.dte_emission_notes || '');

                    // Manejar el switch de emisión DTE
                    if (config.dte_emission_enabled == 1) {
                        $('#dte_emission_enabled_edit').prop('checked', true);
                    } else {
                        $('#dte_emission_enabled_edit').prop('checked', false);
                    }

                    // Configurar la empresa seleccionada
                    var companyOption = new Option(config.name_company, config.company_id, true, true);
                    $('#companyedit').append(companyOption).trigger('change');
    ...
    $('#updateConfigModal form').on('submit', function(e) {

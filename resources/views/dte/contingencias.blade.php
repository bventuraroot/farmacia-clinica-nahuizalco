...
<div class="container-xxl flex-grow-1 container-p-y">
  @if (session('success') || session('warning') || session('error'))
    <div class="mb-3">
      ...
      @if (session('error_details') && count((array)session('error_details')) > 0)
        <ul class="mt-2 small text-danger">
          @foreach ((array)session('error_details') as $err)
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="contingenciasTable" class="table table-striped table-hover align-middle text-nowrap">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Nombre</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            ...
                                            {{ $contingencia->fInicio ? $contingencia->fInicio->format('d/m/Y') : 'N/A' }}
                                        </div>
                                        <div class="mt-1">
                                            <small class="text-muted">Fin:</small><br>
                                            {{ $contingencia->fFin ? $contingencia->fFin->format('d/m/Y') : 'N/A' }}
                                        </div>
                                        @if($contingencia->isVigente())
                                    ...
                                    <td>
                                        <span class="badge bg-info">{{ $contingencia->dtes->count() }}</span>
                                    </td>
                                    <td>
                                        @php
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            @if($contingencia->estado === 'Enviado')
                                                <span class="badge bg-success">Procesada</span>
                                            @elseif($contingencia->estado === 'Rechazado')
                                                <span class="badge bg-danger me-2">Rechazada</span>
                                                <button onclick="confirmarAutorizacion({{ $contingencia->id }}, {{ $contingencia->idEmpresa ?? 1 }})"
                                                        class="btn btn-sm btn-outline-danger"
                                                        title="Reintentar autorización">
                                                    <i class="ti ti-refresh"></i>
                                                    Reintentar
                                                </button>
                                            @elseif(($contingencia->estado === 'En Cola' || $contingencia->estado === 'pendiente') && !$contingencia->selloRecibido)
                                                <button onclick="confirmarAutorizacion({{ $contingencia->id }}, {{ $contingencia->idEmpresa ?? 1 }})"
                                            ...
                                            @else
                                                <span class="badge bg-secondary">{{ $contingencia->estado ?? 'Desconocido' }}</span>
                                            @endif

                                            @if($contingencia->observacionesMsg || $contingencia->estadoHacienda)

...
<body>
    @php
        // Normalización tolerante similar a FAC
        $em = $emisor[0] ?? [];
        $em['ncr'] = $em['ncr'] ?? ($em['nrc'] ?? '');
        if (!isset($em['direccion']) || !is_string($em['direccion'])) {
            // En CRF original dirección se muestra como string; intenta extraer complemento
            $em['direccion'] = $em['direccion']['complemento'] ?? ($em['direccion'] ?? '');
        }
        $emisor[0] = $em;

        $doc0 = $documento[0] ?? [];
        $doc0['versionJson'] = $doc0['versionJson'] ?? ($doc0['versionjson'] ?? ($doc0['version'] ?? ''));
        $doc0['id_doc'] = $doc0['id_doc'] ?? ($doc0['actual'] ?? ($doc0['nu_doc'] ?? ''));
        $documento[0] = $doc0;

        $j = $json ?? [];
        $j['numeroControl'] = $j['numeroControl'] ?? ($j['identificacion']['numeroControl'] ?? '');
        $j['codigoGeneracion'] = $j['codigoGeneracion'] ?? ($j['identificacion']['codigoGeneracion'] ?? '');
        $j['fhRecibido'] = $j['fhRecibido'] ?? ($j['identificacion']['fecEmi'] ?? '');
        $json = $j;

        $cli0 = $cliente[0] ?? [];
        $cli0['ncr'] = $cli0['ncr'] ?? ($cli0['nrc'] ?? '');
        $cliente[0] = $cli0;
    @endphp
        ...
        <tr valign="top">
            <td width=45%>
                <table width="100%">
                    <tr>
                        <td>
                            @php($logoKey = $emisor[0]['ncr'] ?? ($emisor[0]['nrc'] ?? null))
                    ...
                    <!--<tr>
                        <td style="font-size: x-small;">
                            <strong>{{ $emisor[0]["nombreComercial"] ?? ($emisor[0]["nombre"] ?? '') }}</strong>
                        </td>
                    </tr>-->
                    <tr>
                    ...
                    <tr>
                        <td>NRC: {{ $emisor[0]["ncr"] ?? ($emisor[0]["nrc"] ?? '') }}</td>
                    </tr>
                    <tr>
                        <td>Actividad económica: {{ $emisor[0]["descActividad"] ?? '' }}</td>
                    </tr>
                    <tr>
                        <td><strong>Dirección:</strong> {{ is_array($emisor[0]['direccion'] ?? null) ? ($emisor[0]['direccion']['complemento'] ?? '') : ($emisor[0]['direccion'] ?? '') }}<br>
                            {{ isset($emisor[0]['municipio']) ? get_name_municipio($emisor[0]['municipio']) : '' }}, {{ isset($emisor[0]['departamento']) ? get_name_departamento($emisor[0]['departamento']) : '' }}</td>
                    </tr>
                    <tr>
            ...
            <td>
                <table width="100%" style="border:1px solid #000;">
                    ...
                    <tr >
                        ...
                        <td colspan="2">{{ $json["numeroControl"] ?? ($json["identificacion"]["numeroControl"] ?? '') }}</td>
                    </tr>
                    <tr >
                    ...
                    <tr>
                        ...
                        <td>{{ $documento[0]["fechacreacion"] ?? ($json["identificacion"]["horEmi"] ?? '') }}</td>
                        <td><strong>Documento interno No:</strong>{{ $documento[0]["id_doc"] ?? ($documento[0]["actual"] ?? ($documento[0]["nu_doc"] ?? '')) }}</td>
                    </tr>
            ...
            <td width="480px">
                <table width="100%" style="border-top:1px solid #000;">
                    ...
                    <tr>
                        ...
                        <td><strong>NRC:</strong> {{ $cliente[0]["ncr"] ?? ($cliente[0]["nrc"] ?? ($json["receptor"]["nrc"] ?? '')) }}</td>
                    </tr>
                    <tr>
                        <td align="right"><strong>Dirección:</strong></td>
                        <td>{{ is_array($cliente[0]["direccion"] ?? null) ? ($cliente[0]["direccion"]["complemento"] ?? '') : ($cliente[0]["direccion"] ?? ($json["receptor"]["direccion"]["complemento"] ?? '')) }}</td>
            ...
            <table width="100%" style="border-collapse:collapse;margin-top:0;border-spacing: 0 0;" class="cuadro">
                <tr>
                    <td width="490px">
                        <table width="100%">
                            ...
                            <tr>
                                <td>{{ $json["extension"]["nombEntrega"] ?? $usuario->name }}</td>
                                <td>{{ $json["extension"]["docuEntrega"] ?? $usuario->nit }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td>{{ $json["extension"]["nombRecibe"] ?? '' }}</td>
                                <td>{{ $json["extension"]["docuRecibe"] ?? '' }}</td>
                            </tr>
                            <tr>
                ...
                <tr class="cuadro">
                    <td colspan="2" style="font-size:6px;"><span style="margin:0;padding=0;"><center>Condiciones generales de los servicios prestados por
                        {{ $emisor[0]["nombreComercial"] ?? ($emisor[0]["nombre"] ?? '') }}</center><br style="margin:0;padding=0;">
                   </span>
                    </td>
                </tr>
            </table>

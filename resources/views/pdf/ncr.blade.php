        ...
        <tr valign="top">
            <td width=45%>
                <table width="100%">
                    ...
                    <tr>
                        <td>Nombre: RUDY ELENILSON CHINCHILLA AGUILAR</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td>Tipo de establecimiento: {{ $comprobante[0][0]["tipo_establecimiento"] ?? '02' }}
                             - {{ $comprobante[0][0]["nombre_tienda"] ?? ($comprobante[0][0]["nombreComercial"] ?? '') }}
                        </td>
                    </tr>
            ...
            <td width="480px">
                <table width="100%" style="border-top:1px solid #000;">
                    ...
                    <tr>
                        ...
                        <td><strong>NRC:</strong> {{ $comprobante[0][0]["nrc_receptor"] ?? ($comprobante[0][0]["ncr_receptor"] ?? '') }}</td>
                    </tr>
                    <tr>
    ...
    <table width="100%" style="border-collapse:collapse;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>1</td>
                <td>{{ $d["descripcion"] ?? '' }}</td>
                <td align="right">{{FNumero($d["pre_unitario"] ?? 0)}}</td>
                <td align="right">0.00</td>
                <td align="right">{{FNumero($d["imp_int_det"] ?? 0)}}</td>
                <td align="right">{{FNumero($d["no_sujetas"] ?? 0)}}</td>
                <td align="right">{{FNumero($d["excento"] ?? 0)}}</td>
                <td align="right">{{FNumero($d["gravado"] ?? 0)}}</td>
            </tr>
            @endforeach
        </tbody>
        ...
        <div class="footer" style="position: absolute; bottom: 0;border-spacing: 0 0;border-collapse:collapse;margin-top:0;">
            <table width="100%" style="border-collapse:collapse;margin-top:0;border-spacing: 0 0;" class="cuadro">
                <tr>
                    <td width="490px">
                        <table width="100%">
                            ...
                            <tr>
                                <td>{{ $comprobante[0][0]["nombEntrega"] ?? $usuario->name }}</td>
                            ...
                            <tr>
                                <td width="245px">
                                    <table width="100%">
                                        <tr>
                                            <td align="right">{{FNumero($comprobante[0][0]["credito"] ?? 0)}}</td>
                                            <td align="right">{{FNumero($comprobante[0][0]["contado"] ?? 0)}}</td>
                                            <td align="right">{{FNumero($comprobante[0][0]["tarjeta"] ?? 0)}}</td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>
                    ...
                    <td style="border:1px solid #000;" width="230px">
                        ...
                        <table style="border-spacing: 0 0;">
                            <tr>
                                ...
                                <td align="right" width="50px" class="sumas">{{FNumero($comprobante[0][0]["tot_nosujeto"] ?? 0)}}</td>
                                <td align="right" width="50px" class="sumas">{{FNumero($comprobante[0][0]["tot_exento"] ?? 0)}}</td>
                                <td align="right" width="50px" class="sumas">{{FNumero($comprobante[0][0]["tot_gravado"] ?? 0)}}</td>

                            </tr>
                            <tr>
                                <td colspan="3" width="160px">Suma total de operaciones</td>
                                <td align="right" class="cuadro-izq">{{FNumero($comprobante[0][0]["subTotalVentas"] ?? 0)}}</td>
                            ...
                            <tr>
                                ...
                                <td align="right" class="cuadro-izq">{{FNumero($comprobante[0][0]["iva"] ?? 0)}}</td>

                            </tr>
                            <tr>
                                <td colspan="3">Sub-Total</td>
                                <td align="right" class="cuadro-izq">{{FNumero($comprobante[0][0]["subTotal"] ?? 0)}}</td>

                            </tr>
                            <tr>
                                <td colspan="3">IVA Percibido</td>
                                <td align="right" class="cuadro-izq">{{FNumero($comprobante[0][0]["ivaPerci1"] ?? 0)}}</td>

                            </tr>
                            <tr>
                                <td colspan="3">IVA Retenido</td>
                                <td align="right" class="cuadro-izq">{{FNumero($comprobante[0][0]["ivaRete1"] ?? 0)}}</td>
                            ...
                            <tr>
                                ...
                                <td align="right" class="cuadro-izq">{{FNumero($comprobante[0][0]["totalNoGravado"] ?? 0)}}</td>

                            </tr>
                            <tr>
                                <td colspan="3" ><strong>TOTAL A PAGAR</strong></td>
                                <td align="right" class="cuadro-izq"><strong>{{FNumero($comprobante[0][0]["totalPagar"] ?? 0)}}</strong></td>
                ...
                <tr class="cuadro">
                    ...
                    • {{ $comprobante[0][0]["nombre_empresa"] ?? ($comprobante[0][0]["nombreComercial"] ?? '') }}. declara expresamente que actúa como agente representante o distribuidor de
                    los
                    ...
                    {{ $comprobante[0][0]["nombre_empresa"] ?? ($comprobante[0][0]["nombreComercial"] ?? '') }}, no tiene responsabilidad alguna en casos de muerte o lesiones de los
                    ...
                    {{ $comprobante[0][0]["nombre_empresa"] ?? ($comprobante[0][0]["nombreComercial"] ?? '') }}, explicaron cada una de las condiciones descritas anteriormente, entendiéndolas
